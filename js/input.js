...
(function () {
  ...
  let autoGas = false, smart = false, control = "pad", wheelSteer = 0;
  ...
  I.resolve = function (carAngle) {
    ...
    if (control === "wheel") { // steering wheel: relative left/right (like a car)
      if (wheelSteer !== 0) { steer = wheelSteer; turn = Math.abs(wheelSteer); }
    } else if (pad.active) { // directional joystick steers toward the pushed heading
  ...
  I.reset = function () {
    pad.x = pad.y = 0; pad.active = false; wheelSteer = 0;
  ...
  // steering wheel: drag left/right to steer; springs back to centre on release
  function bindWheel() {
    ...
    const apply = (x) => {
      wheelSteer = clamp((x - startX) / 80, -1, 1);
      cv.style.transition = "none";
      cv.style.transform = `rotate(${wheelSteer * 135}deg)`;
    };
    const release = () => {
      id = null; wheelSteer = 0;
      cv.style.transition = "transform .16s ease-out";
      cv.style.transform = "rotate(0deg)";
    };
    if (window.PointerEvent) {
      zone.addEventListener("pointerdown", (e) => { id = e.pointerId; startX = e.clientX; try { zone.setPointerCapture(e.pointerId); } catch (_) {} apply(e.clientX); e.preventDefault(); }, { passive: false });
      zone.addEventListener("pointermove", (e) => { if (e.pointerId === id) { apply(e.clientX); e.preventDefault(); } }, { passive: false });
      ...
      zone.addEventListener("touchstart", (e) => { const t = e.changedTouches[0]; id = t.identifier; startX = t.clientX; apply(t.clientX); e.preventDefault(); }, { passive: false });
      zone.addEventListener("touchmove", (e) => { for (const t of e.changedTouches) if (t.identifier === id) apply(t.clientX); e.preventDefault(); }, { passive: false });
