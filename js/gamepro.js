...
(function () {
  ...
  class RacePro {
    ...
    _renderIso() {
      ...
      const pg = (x, y) => [ox + (x - camX) * zoom, oy + (y - camY) * zoom * ISO]; // ground projection
      const wallH = 60 * zoom, theme = this.track.theme || { ground: "#3f6b2e", dirt: "#b07a45" };

      // sky / ground backdrop
      ctx.setTransform(1, 0, 0, 1, 0, 0);
      const grad = ctx.createLinearGradient(0, 0, 0, H);
      grad.addColorStop(0, shade(theme.ground, 0.7)); grad.addColorStop(1, shade(theme.ground, 1.05));
      ctx.fillStyle = grad; ctx.fillRect(0, 0, W, H);

      // foreshortened ground plane (the baked course) + skid decals + pickups
      ctx.save();
      ctx.translate(ox, oy); ctx.scale(zoom, zoom * ISO); ctx.translate(-camX, -camY);
      ...
      for (const side of [Wd / 2 + 11, -(Wd / 2 + 11)]) {
      ...
      for (const s of segs) {
        ...
        const t0 = [b0[0], b0[1] - wallH], t1 = [b1[0], b1[1] - wallH];
    ...
    _drawCarIso(ctx, car, pg, zoom, ISO) {
      ...
      if (car.isPlayer) {
        const k = 1 / zoom, pulse = Math.sin(this.time * 6);
        ctx.save(); ctx.translate(sx, sy - lift);
        ctx.strokeStyle = "rgba(255,238,0,0.97)"; ctx.lineWidth = 4;
        ctx.beginPath(); ctx.arc(0, 0, 26 + pulse * 2, 0, Math.PI * 2); ctx.stroke();
        const ay = -46 + pulse * 4;
        ctx.beginPath(); ctx.moveTo(-15, ay - 15); ctx.lineTo(15, ay - 15); ctx.lineTo(0, ay + 4); ctx.closePath();
    ...
    _drawBridgeIso(ctx, pg, zoom) {
      const bz = this.track.bridge, mid = ((bz[0] + bz[1]) / 2) * this.N;
      const p = pointAtProgress(this.pts, mid), tan = tangentAtProgress(this.pts, mid);
      const half = this.track.width / 2 + 8, len = this.track.width * 2.3, dh = 30 * zoom;
      ...
      const base = cs.map((q) => pg(q.x, q.y)), top = base.map((q) => [q[0], q[1] - dh]);
