...
(function () {
  ...
  class RacePro {
    ...
    _buildEnvProps() {
      ...
      // how far the vehicle can stray from the centreline (+ a margin); NO prop
      // footprint may intrude inside this, so the craft never flies/sails into one.
      const edge = (this.vehKind === "heli" ? Wd / 2 + 10 : Wd / 2 + 6);
      ...
      for (let gx = b.minX - 130; gx <= b.maxX + 120; gx += step) {
        for (let gy = b.minY - 130; gy <= b.maxY + 120; gy += step) {
          ...
          const pr = big(), half = Math.max(pr.w, pr.d) / 2;
          // the whole footprint must clear the lane (and not sit in a neighbour lane)
          if (closestOnLoop(this.pts, x, y).dist - half < edge + 30) continue;
      ...
      if (heliLand) {
        const n = this.N, gap = env === "urban" ? 24 : env === "mountain" ? 30 : 26;
        for (let f = 0; f < 1; f += 0.03) {
          ...
          for (const side of [1, -1]) {
            const pr = big(), half = Math.max(pr.w, pr.d) / 2, off = edge + gap + half; // inner face = edge + gap, clear of the flight lane
            if (env === "urban") pr.height *= 0.72; // shorter near the lane so it doesn't occlude
            const x = p.x + nx * side * off, y = p.y + ny * side * off;
            if (closestOnLoop(this.pts, x, y).dist - half < edge) continue; // footprint mustn't reach a neighbouring corridor
            props.push(Object.assign({ x, y }, pr));
          }
        }
      }
    ...
    _trackLogic(car, dt, racing) {
      ...
      if (this.vehKind === "heli") {
        ...
        // speed and nudges you back in; the hard limit stays clear of the obstacle
        // line so the heli never flies into a building/rock/tree.
        const soft = this.track.width / 2 - 10, hard = this.track.width / 2 + 4;
        if (c.dist > soft) {
